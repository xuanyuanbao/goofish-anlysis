...
class Database:
    ...
    def keyword_count(self) -> int:
        with self.connect() as connection:
            row = connection.execute("SELECT COUNT(1) AS cnt FROM keyword_config").fetchone()
        return int(row["cnt"])
    ...
    def replace_snapshots(self, snapshot_date: date, items: Iterable[CrawledItem]) -> int:
        item_list = list(items)
        if not item_list:
            return 0
        keywords = sorted({item.keyword for item in item_list})
    ...
    def replace_daily_stats(self, stat_date: date, stats: Iterable[DailyKeywordStat]) -> int:
        rows = list(stats)
        if not rows:
            return 0
        insert_rows = [
    ...
    def replace_item_scores(self, stat_date: date, rows: Iterable[DailyItemScore]) -> int:
    ...
    def fetch_daily_stats_between(self, start_date: date, end_date: date) -> list[dict[str, object]]:
        with self.connect() as connection:
            rows = connection.execute(
                """
                SELECT stat_date, keyword, category, item_count, avg_price, min_price,
